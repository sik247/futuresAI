...
const CommentTiptap: React.FC<{
  isComment?: boolean;
  isNotice?: boolean;
}> = ({ isComment = false, isNotice = false }) => {
  const [imageLoading, setImageLoading] = useState(false);
  const { commentText, setCommentText, insertText } = commentTextStore();
  ...
  //       totalImgHtml =
  //         totalImgHtml +
  //         `<img src="https://otyxxevpzfftduqhgapx.supabase.co/storage/v1/object/public/dongsaroma-attachment/${data.path}" alt="image"/>`;
  //     }
  ...
  useEffect(() => {
    if (!editor) return;
    if (commentText !== editor.getHTML())
      editor.commands.setContent(commentText);
  }, [commentText, editor]);
  ...
  return (
    <>
      ...
      <div className="w-full border shadow-sm rounded-md flex flex-col">
        <div className="p-4 w-full flex gap-4 border-b items-center overflow-x-scroll overflow-y-hidden">
          {/* <Select
            defaultValue="Medium"
            value={
