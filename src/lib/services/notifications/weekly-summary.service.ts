...
export async function sendWeeklySummary(): Promise<boolean> {
  try {
    ...
    const dateRange = `${formatDateKo(weekAgo)} ~ ${formatDateKo(today)}`;
    ...
    msg += `\n<b>주요 지표</b>\n`;
    if (btc) {
      msg += `BTC: $${formatPrice(btc.current_price)} (주간 ${(btc.price_change_percentage_7d_in_currency ?? 0) >= 0 ? "+" : ""}${(btc.price_change_percentage_7d_in_currency ?? 0).toFixed(1)}%)\n`;
    }
    if (eth) {
      msg += `ETH: $${formatPrice(eth.current_price)} (주간 ${(eth.price_change_percentage_7d_in_currency ?? 0) >= 0 ? "+" : ""}${(eth.price_change_percentage_7d_in_currency ?? 0).toFixed(1)}%)\n`;
    }
    msg += `Top 30 거래량: $${(totalVol / 1e9).toFixed(1)}B\n`;
...
async function generateWeeklyAnalysis(
  ...
  try {
    ...
    const prompt = `한국어 주간 크립토 시장 분석을 5-6문장으로 작성하세요.
